    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (!existing) {
        existing = {
          ...
          isFile,
          children: [],
          ...(isFile && { additions: file.additions, deletions: file.deletions })
        };
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center gap-1 shrink-0 pr-1">
          {(node.additions ?? 0) > 0 && (
            <span className="text-[10px] text-green-600 dark:text-green-400">+{node.additions}</span>
          )}
          {(node.deletions ?? 0) > 0 && (
            <span className="text-[10px] text-red-600 dark:text-red-400">-{node.deletions}</span>
          )}
        </div>
      </div>
    );
  ...
  return (
    <div>
      <button
        ...
        className="flex items-center gap-1.5 py-0.5 text-xs text-muted-foreground hover:text-foreground w-full"
        style={{ paddingLeft: `${depth * 12 + 4}px` }}
      >
        {expanded ? (
          <FolderOpen className="w-3 h-3 shrink-0 text-yellow-600 dark:text-yellow-500" />
        ) : (
          <Folder className="w-3 h-3 shrink-0 text-yellow-600 dark:text-yellow-500" />
        )}
  ...
  const toggleExpand = async (e: React.MouseEvent) => {
    ...
    if (files === null) {
      ...
      try {
        ...
        const { files: parsedFiles } = parseDiff(diff.diff);
    ...
    <div className="rounded-md hover:bg-secondary/50 transition-colors">
      <div className="flex items-start gap-2 px-2 py-2">
        ...
        <button
          ...
          className="mt-0.5 shrink-0 w-4 h-4 flex items-center justify-center text-muted-foreground hover:text-foreground"
        >
          {loading ? (
      ...
      {/* Expanded File Tree */}
      {expanded && files !== null && files.length > 0 && (
        <div className="px-2 pb-2 pl-8">
          <FileTree files={files} />
        </div>
      )}
      {expanded && files !== null && files.length === 0 && !loading && (
        <div className="px-2 pb-2 pl-8">
          <p className="text-xs text-muted-foreground italic">No files changed</p>
        </div>
      )}
    </div>
  );
...
export function HistoryTab({ repositoryId }: HistoryTabProps) {
  ...
  const handleViewDetails = async (commit: GitCommit) => {
